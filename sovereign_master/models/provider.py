...
class BaseModelProvider(ABC):
    ...
    def health(self):
        return {"provider": self.name, "available": False, "status": "not_configured", "capabilities": asdict(self.capabilities)}
    def metadata(self):
        return {"name": self.name, "timeout_seconds": self.timeout_seconds, "capabilities": asdict(self.capabilities)}
...
class LocalFallbackProvider(BaseModelProvider):
    ...
    def health(self):
        return {**super().health(), "status": "fallback_only", "note": "Not a generative AI model"}
...
class OpenAICompatibleProvider(BaseModelProvider):
    ...
    def _request(self, prompt):
        ...
        req = urllib.request.Request(self.base_url + "/chat/completions", body, {"Content-Type":"application/json", "Authorization":"Bearer " + self.api_key})
        with urllib.request.urlopen(req, timeout=self.timeout_seconds) as response:
            data = json.loads(response.read().decode())
        return data["choices"][0]["message"]["content"]
    def generate(self, prompt, context=None): return self._request(prompt)
    def health(self):
        configured = bool(self.api_key and self.model)
        return {**super().health(), "available": configured, "status": "configured" if configured else "missing_configuration", "model": self.model}

class ProviderFactory:
    @staticmethod
    def create_from_environment():
        selected = os.getenv("MODEL_PROVIDER", "").strip().lower()
        if selected in {"openai", "openai_compatible"}: return OpenAICompatibleProvider()
